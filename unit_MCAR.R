# created: 07/07
# modified: 07/09 (prior for alpha and gamma centered at truth)
...
# Use margin of X as the last step in MCMC
...
ANWC_unit_full<-function(niter = 10000,burnin = 5000, data = sub_dat, alpha = alpha_s, 
  ...
  b0_alpha <- alpha; b0_alpha[] <- c(0.3,-0.5,0)
  sigma0_alpha <- diag(alpha); diag(sigma0_alpha) <- 1
  b0_gamma <- gamma; b0_gamma[] <- c(-0.25,0.1,-1.5)
...
save(resultListANWC,sub_dat,ratio,MI_dataANWC,pop_sd_HT,file = paste("./sim0_new/Mis_",run_index,".RData",sep=""))
