...
# update Rx per iteration
# impute missing X for U==1
# fully Bayesian
...
ANWC_unit_full<-function(niter = 10000,burnin = 5000, data = sub_dat, alpha = alpha_s, beta = beta_s,
  ...
  b0_beta <- beta; b0_beta[] <- 0
  sigma0_beta <- diag(beta); diag(sigma0_beta) <- 1
  ...
  for (i in 1:niter){
    ...
    # p_X1 <- pnorm(alpha[1]+alpha[2]*Y) # changed generating model of X1 to exclude weight
...
save(resultListANWC,sub_dat,ratio,MI_dataANWC,pop_sd_HT,file = paste("./unit7/Mis_",run_index,".RData",sep=""))
