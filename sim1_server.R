...
ANWC_unit_full<-function(niter = 10000,burnin = 5000, data = sub_dat, alpha = alpha_s, 
                         gamma = gamma_s, nu = nu_s, pop_mean = pop_mean_HT, pop_sd = pop_sd_HT, weight_opt = "mean"){
  X1 <- sub_dat$X1
  W <- sub_dat$W
  Rx <- sub_dat$Rx
  Y <- sub_dat$Y
  ...
  for (i in 1:niter){
    ...
    # Update nu
    # update V|nu,W
    U_mat <- model.matrix(U~pbeta) # include weight in Y model
    V <- U
    V_mean <- U_mat%*%t(nu)
...
getResults_unit <- function(dataMI_X,dataMI_Y,alpha_len, gamma_len,beta_len, nu_len, sub_dat){
  ...
  for (i in 1:n){
    ...
    ans_g[i,] <- coef(m2)
    ul_g[i,] <- diag(vcov(m2))
    m4 <- svyglm(U~pi,design = mydesign,family = quasibinomial(link = "probit"))
    ans_nu[i,] <- coef(m4)
  ...
  return(list(alpha = ans, gamma = ans_g, beta = ans_b, nu = ans_nu, total_mean = mean(total), total_var = T_L_t, 
...
gamma_s <- rnorm(3)
nu_s <- rnorm(2)
testListANWC <- ANWC_unit_full(weight_opt = "bs")
...
save(resultListANWC,sub_dat,ratio,MI_dataANWC,pop_sd_HT,file = paste("./sim1_2/bs/Mis_",run_index,".RData",sep=""))
