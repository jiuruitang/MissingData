# created: 04/18
# modified: 04/18
# unit nonresponse trial 1 to be run on server
...
doGibbsANWCPoisson_unit<-function(niter = 10000,burnin = 5000, data = sub_dat, alpha = alpha_s, gamma = gamma_s, pop_mean = pop_mean_HT, pop_sd = pop_sd_HT){
  X1 <- sub_dat$X1
  W <- sub_dat$W
  Rx <- sub_dat$Rx
  Y <- sub_dat$Y
  ...
  # for U = 1, impute Rx (assuming MAR)
  m_Rx <- glm(Rx~Y, data = sub_dat[which(U == 0),],family=binomial(probit))
  p_Rx <- predict(m_Rx,newdata = data.frame(Y=Y[which(U == 1)]),type = "response")
  Rx[which(U == 1)] <- rbinom(n_unit,1,prob = p_Rx)
  
  # now, for Rx = 1, impute X to get a starting point
  n_mis <- sum(Rx)
  ...
  for (i in 1:niter){
    ...
    # Update Rx for U = 1 only
    pi_Rx <- pnorm(YX_mat%*%t(gamma))
    Rx[which(U==1)] <- rbinom(n_unit,1,p = pi_Rx[which(U==1)])
    n_mis <- sum(Rx)
    ...
    if (i > burnin){
      ...
      Y_impute[i-burnin,] <- Y
      Rx_impute[i-burnin,] <- Rx
    }
  }
  XL <- X1_impute[seq(1,(niter-burnin),100),]
  YL <- Y_impute[seq(1,(niter-burnin),100),]
  RxL <- Rx_impute[seq(1,(niter-burnin),100),]
  return(list(gamma = GAMMA,alpha = ALPHA,MI_dataX = XL, MI_dataY = YL, MI_dataRx = RxL, acceptRatio = X1_accepted/niter))
}
...
alpha_s <- rnorm(3)
gamma_s <- rnorm(3)
testListANWC <- doGibbsANWCPoisson_unit()
...
MI_dataANWC <- data.frame(X = testListANWC$MI_dataX, Y = testListANWC$MI_dataY, Rx = testListANWC$MI_dataRx)

save(resultListANWC,sub_dat,ratio,MI_dataANWC,pop_sd_HT,file = paste("./unit1/Mis_",run_index,".RData",sep=""))
