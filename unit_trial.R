# created: 04/18/20
# modified: 04/18/20
# first trial of implementing unit nonresponse
# assume unit nonresponse follows MCAR
...
gen_pop_unit <- function(N=length(nonzero_W),HTW = nonzero_W,alpha = c(0.3,-0.5),
                            gamma = c(-0.25,0.1,-1.5),beta=c(0.5,-0.15),nu = -1.2){
  # pi <- rbeta(N,2,5) 
  W <- HTW
  pi <- 1/W
  Y <- W
...
pop_dat <- gen_pop_unit() # 11% unit nonresponse
